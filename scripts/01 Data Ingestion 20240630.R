...
input_path = "data/source_files/fatalities_20240630_src.txt"
output_path = "data/fatalities_20240630.csv"
...
df = data.frame(
  id     = lines[seq(from= 9, by=7, to=197303)],
  name   = lines[seq(from=10, by=7, to=197303)],
  gov_id = lines[seq(from=11, by=7, to=197303)],
  dob    = lines[seq(from=12, by=7, to=197303)],
  sex    = lines[seq(from=13, by=7, to=197303)],
  age    = lines[seq(from=14, by=7, to=197303)],
  source = lines[seq(from=15, by=7, to=197303)]
)
...
df$sex %>% str_detect("^(أنثى|ذكر)$") %>% all

df$age %>% str_detect("^[0-9]+$") %>% all

df$source %>% str_detect("^(تبليغ ذوي الشهداء|سجلات وزارة الصحة)$") %>% all
